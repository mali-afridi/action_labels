...
class VideoLabelApp:
        ...
        try:
            with open(file_path, 'r') as file:
                for line in file:
                    ...
                    if len(parts) == 2:
                        frame_index, label = parts
                        if frame_index.isdigit():
                            self.labels[int(frame_index)] = label
        except Exception as e:
            messagebox.showerror("Error", f"Failed to import labels: {e}")
            return

        # Update the label listbox and dropdown with the imported labels
        unique_labels = set(self.labels.values())
        self.label_listbox.delete(0, tk.END)
        for label in unique_labels:
            self.label_listbox.insert(tk.END, label)
        self.update_dropdown()
        messagebox.showinfo("Imported", "Labels imported successfully.")
    def create_widgets(self):
            ...
            self.import_labels_button = tk.Button(
                self.left_frame, text="Import Labels", command=self.import_labels
            )
            self.import_labels_button.pack()
